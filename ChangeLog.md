...
- NEW - 13/09/2026 - API Ajout de la possibilité de passer un financement en actif (STATUS_RUNNING) via POST fundings/{id}/run, aux mêmes conditions que le bouton "BtnRunning" de funding_card.php (droit funding:write, origine <> propal, statut STATUS_ACCEPT, passage par setRun déclenchant FUNDING_RUNNING)
